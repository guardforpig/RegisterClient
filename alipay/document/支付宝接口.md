# 1.注意事项
v2.0 回调加入了total_amount,refund_fee,gmt_refund,gmt_payment
## 1.时间格式
...
| refund_fee | Long          | 退款通知中，返回总退款金额(这里应该是请求的退款金额)，单位为人民币（分） 如果退款失败或者不是退款回调则为空                                                                                                            |
...
| gmt_refund_pay  | LocalDateTime   | 退款时间   |
